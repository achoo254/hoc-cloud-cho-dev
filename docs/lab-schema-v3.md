...
- FE parse lab content qua Zod schema tại `app/src/lib/schema-lab.ts` — Zod sẽ throw khi missing/invalid field.
- Không còn seed path từ fixtures; lab content insert trực tiếp vào MongoDB (UI editor hoặc Mongoose query).
- Legacy HTML validator (`scripts/validate-lab-schema.js`) + fixture validator (`scripts/validate-lab-fixtures.js`) đã xóa sau khi migrate sang MongoDB + Vite SPA.
...
- Runtime Zod schema: `app/src/lib/schema-lab.ts`
