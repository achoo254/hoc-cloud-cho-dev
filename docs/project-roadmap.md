...
### Bài Tập (Exercises) — owner-gated (P0.5, 2026-06-02)
- Collection `exercises` riêng (độc lập với labs), private chỉ owner (`OWNER_EMAIL`/`VITE_OWNER_EMAIL`). Mỗi bài = Đề bài → Hướng dẫn → Demo thật.
- API `GET /api/exercises` + `/:slug` (owner-gated: `require-owner.js`). FE: nav "Bài Tập" + `/exercises` + `/exercise/:slug`.
