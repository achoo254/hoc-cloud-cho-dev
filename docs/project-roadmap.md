...
|-------|--------|---------|
| P0 · Content pipeline & schema | ✅ Done | Lab JSON v3 schema, Zod validation, fixtures → TS + SQLite sync |
...
| P4 · Search (FTS5 + client fallback) | ✅ Done | `/api/search` với bm25 + `<mark>` highlight, MiniSearch fallback |
...
- Generators: `fixtures-to-ts.mjs`, `generate-labs-index.mjs`, `generate-search-index.mjs`
...
### Search (P4)
- SQLite FTS5 virtual table với bm25 ranking
- `<mark>` highlight trong snippet
- Client-side MiniSearch fallback
- Command palette (`cmdk`) trong UI
...
- Có nên thay better-sqlite3 bằng LibSQL/Turso để multi-region không?
