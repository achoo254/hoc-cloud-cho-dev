...
- Module `01-networking` (8 labs): `arp`, `dhcp`, `dns`, `http`, `icmp-ping`, `subnet-cidr`, `tcp-ip-packet-journey`, `tcp-udp`
- Module `02-observability` (1 lab): `victorialogs` (thêm 2026-06-11)
- MongoDB (labs collection) = single source cho lab content. Meilisearch index auto-sync qua Mongoose post-save hooks. FE đọc runtime qua `/api/labs` — không bundled content, không fixtures pipeline.
- Bulk Meili re-sync khi ghi thẳng Mongo: `server/scripts/sync-meili-index.js` (chạy với env trỏ Meili môi trường cần cập nhật).

### Bài Tập (Exercises) — public (P0.5, 2026-06-02; mở public 2026-06-03)
- Collection `exercises` riêng (độc lập với labs), **public** (ai cũng xem, như labs/search). Mỗi bài = Đề bài → Hướng dẫn → Demo thật.
- API `GET /api/exercises` + `/:slug` (không gate). FE: nav "Bài Tập" + `/exercises` + `/exercise/:slug`.
- 3 bài đầu (`syslog`, `linux-boot-process`, `linux-swap`) chuyển từ labs sang. Tạo bài mới: `server/scripts/migrate-linux-labs-to-exercises.js` pattern / seed.
